...
// sur `player_evaluation`) doit le transmettre en query (`?teamId=`) pour
...
@Controller('clubs/:clubId/players/:playerId/evaluations')
export class PlayerEvaluationsController {
  constructor(
    private readonly playerEvaluationsService: PlayerEvaluationsService,
  ) {}

  @RequirePermission('player_evaluation', 'CREATE')
  @Post()
  create(
    ...
    @Body() dto: CreatePlayerEvaluationDto,
    @Query('teamId') teamId?: string,
  ) {
    return this.playerEvaluationsService.create(
      clubId,
      playerId,
      member.id,
      dto,
  ...
  @RequirePermission('player_evaluation', 'READ')
  @Get()
  findAll(
    @Param('clubId', ParseIntPipe) clubId: number,
    @Param('playerId', ParseIntPipe) playerId: number,
    @CurrentMember() member: Member,
    @CurrentPermissionScope() scope: PermissionScope,
    @Query() query: FindPlayerEvaluationsQueryDto,
    @Query('teamId') teamId?: string,
  ) {
    return this.playerEvaluationsService.findAllByPlayer(
      clubId,
      playerId,
      {
        memberId: member.id,
  ...
  @RequirePermission('player_evaluation', 'UPDATE')
  @Patch(':id')
  update(
    @Param('clubId', ParseIntPipe) clubId: number,
    @Param('playerId', ParseIntPipe) playerId: number,
    @Param('id', ParseIntPipe) id: number,
    @Body() dto: UpdatePlayerEvaluationDto,
    @CurrentMember() member: Member,
    @CurrentPermissionScope() scope: PermissionScope,
    @Query('teamId') teamId?: string,
  ) {
    return this.playerEvaluationsService.update(clubId, playerId, id, dto, {
      memberId: member.id,
      scope,
      teamId: teamId !== undefined ? Number(teamId) : undefined,
    });
  ...
  @RequirePermission('player_evaluation', 'DELETE')
  @Delete(':id')
  remove(
    @Param('clubId', ParseIntPipe) clubId: number,
    @Param('playerId', ParseIntPipe) playerId: number,
    ...
    return this.playerEvaluationsService.remove(clubId, playerId, id, {
      memberId: member.id,
      scope,
      teamId: teamId !== undefined ? Number(teamId) : undefined,
    });
