...
| B6 | Retour utilisateur (2026-07-17) : fusion Convocations+Composition en un onglet unique "Avant-match" à colonnes (`PreMatchTab`), terrain SVG glisser-déposer fait maison pour la composition (`usePitchInteractions`/`PitchSvg`, réutilise `POSITION_PITCH_SPOTS` de la fiche joueur) plutôt qu'une liste avec sélecteurs, banc auto-peuplé depuis les convocations acceptées (`AddToLineupDialog` supprimé). Schéma : `MatchLineup.pitchSpotId` (+ contrainte `@@unique([matchId, pitchSpotId])`) pour épingler chaque joueur à un point précis du terrain, distinct de `position` ✅ |
| B7 | Retour utilisateur (2026-07-17, capture d'écran) : colonne Convocations trop large et terrain démesuré (viewBox carré non borné) faisaient perdre le Banc, affiché sous le terrain. Passage à 3 colonnes inégales (Convocations/Composition/Banc, grid `[0.9fr_1.3fr_0.8fr]`), terrain borné (`max-w-xs`), Banc isolé dans sa propre colonne (`BenchList`, toujours visible) ✅ |
| B8 | Retour utilisateur (2026-07-17) : (1) systèmes tactiques (`Match.formation`, `frontend/src/lib/formations.ts` — ~6 formations, terrain désormais piloté par la formation choisie plutôt que par `POSITION_PITCH_SPOTS` générique, changement de système conserve les joueurs dont le point existe toujours) ; (2) liste "Titulaires" toujours affichée supprimée, remplacée par une barre d'actions contextuelle au clic sur un titulaire (numéro de maillot, capitaine, retrait) ; (3) capitaine du match (`MatchLineup.isCaptain`, réservé aux titulaires, un seul par match maintenu par `MatchLineupsService`) ; (4) statut "non retenu" piloté depuis l'UI (`LineupStatus.NON_CONVOQUE`, section séparée du banc) ; (5) colonne Convocations bornée avec défilement interne (chaîne `lg:h-full`/`lg:min-h-0`/`lg:flex-1` reprise de `players/[playerId]/page.tsx`) — page ne défile plus entièrement pour une longue liste ✅ |

| B9 | Retour utilisateur (2026-07-17) : (1) glisser-déposer peu fiable entre deux points du terrain — suivi du geste basculé sur des écouteurs `window` posés au `pointerdown` plutôt que `setPointerCapture` par élément, plus robuste ; (2) retour visuel du glisser renforcé (jeton flottant avatar, point survolé coloré vert/rouge selon validité, Banc mis en évidence) ; (3) initiales et numéro de maillot affichés ensemble sur chaque point occupé (avant : l'un remplaçait l'autre) ; (4) terrain dimensionné par la hauteur disponible (`h-full`/`aspect-square`) plutôt que par une largeur bornée (`max-w-xs`, devenu trop petit après B7) ✅ |
| B10 | Retour utilisateur (2026-07-17, CSV fourni par l'utilisateur) : catégorie d'âge d'équipe (`Team.category`, enum `TeamCategory` — U9 à Seniors) et format de jeu (`GameFormat` 4 à 11, `Championship.gameFormat` avec défaut ELEVEN, `Match.gameFormat` en surcharge nullable, même pattern que les périodes de jeu) ; `frontend/src/lib/formations.ts` couvre les 7 formats (~50 dispositifs, généré via `buildFormation`/bandes Y/presets X plutôt que des coordonnées écrites à la main) ; formulaires équipe/championnat/match mis à jour, format de jeu préempli depuis la catégorie à la création d'un match direct (`CATEGORY_DEFAULT_GAME_FORMAT`) ✅ |
| B11 | Retour utilisateur (2026-07-18) : édition d'un match existant (bouton "Modifier" sur la fiche match, `MatchEditDialog` — horaires, lieu, description, adversaire/coupe/domicile-extérieur sauf pour un match Championnat, format de jeu) via `PATCH /matches/:id` déjà transactionnel côté backend, aucun changement backend nécessaire ; corrections composition : rafraîchissement de la liste des dispositifs propagé depuis l'édition du match (`matchRefreshKey`, absent jusque-là), identifiants de dispositif sans chiffre de gardien pour les formats sub-11 (`3-3-2` et non `1-3-3-2`, même convention que le 11 contre 11), tri numérique de la liste déroulante, ajout du dispositif `2-5-1` (9 contre 9) ✅ |

Tests à la fin de la Partie B : 742 tests backend + 588 tests frontend.
...
| C3 | Backend clôture (`MatchesService.close`, `POST .../matches/:id/close`, gardé par `match_period UPDATE` — pas `match UPDATE`, cohérent avec "Clore le match" ❌ pour AdminClub malgré son CRUD complet sur `match`) — calcul du score depuis `MatchEvent` `GOAL`/`OWN_GOAL` (`PENALTY_SCORED/MISSED` jamais comptés, réservés aux tirs au but), écrit sur `ChampionshipMatch` (+ `status = FINISHED`, seul flux qui déclenche le recalcul du classement) ou `Match` selon le type. Rejette si une période est encore ouverte ou si le match est déjà terminé/annulé/reporté. **Gap connu, non traité ici** : `ChampionshipMatchesService.update` (saisie manuelle) n'a aucun garde-fou empêchant d'écraser score/statut d'un `ChampionshipMatch` lié à un `Match` en cours de gestion live — l'intention documentée ("la clôture live reste l'unique flux qui fait passer un Match à FINISHED") n'est pas encore appliquée côté saisie manuelle ✅ |
